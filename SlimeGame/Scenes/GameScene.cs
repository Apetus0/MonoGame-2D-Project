...
namespace SlimeGame.Scenes
{
    public class GameScene : Scene
    {
        ...
        // Reference to the titlemap to draw
        ...
        public override void Initialize()
        {
            ...
            // Initalize the UI for the game scene
            InitializeUI();
        ...
        {
            // Clear out any previous UI element incaase we came here 
        ...
        {
            // Player has chosen to quit, so return back to the title screen
            Core.ChangeScene(new TitleScene());
        }

        private void InitializeNewGame()
        {
            // Calculates the position for the slime, which will be at the center
        ...
        private void CollisionChecks()
        {
            ...
            // Next check if the slime is colliding with the wall by validing if 
            ...
            // it is within the bounds of the room. If it is outside the room,
            // bounds, then it collided with a wall and the bat should bounce off that wall
            ...
            else if (batBounds.Left < _roomBounds.Left)
            {
        ...
        {
            // Calculate the position that is the center of the bounds
            // of the room
            ...
            if(Math.Abs(centerToSlime.X) > Math.Abs(centerToSlime.Y))
            {
                ...
                // position will be a random position between the top and bottom walls
                ...
                {
                    // The slime is closer to the right side wall, so place the bat
            ...
            else
            {
                ...
                // position will be a random position between the left and right walls
                ...
                {
                    // The slime is closer to the top wall, so place the bat on
                    // the bottom wall
                    newBatPosition.Y = _roomBounds.Top + padding;
                }
                else
                {
                    // The slime is closer to the bottom wall, so place the bat
                    // on the top wall
                    newBatPosition.Y = _roomBounds.Bottom - padding * 2;
                }
            }
            //Assign the bat the new position
        ...
        private void TogglePause()
        {
            ...
            {
                /// We're pausing the game, so show the pause panel
                _ui.ShowPausePanel();
