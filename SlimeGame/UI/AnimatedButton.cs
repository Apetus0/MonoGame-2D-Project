...
namespace SlimeGame.UI
{
    ...
    internal class AnimatedButton : Button
    {
        public AnimatedButton(TextureAtlas atlas)
        {
            ...
            // A nine-slice allows the button to stretch while preserving corner appearances 
            ...
            // Get the multi-frame animation for the focused button statre from the atlas
            Animation focusedAtlasAnimation = atlas.GetAnimation("focused-button-animation");

            // Create an animation chain for the focused state using all frames from the atlas
            ...
            // Asign both animation chains to the nine-slice background
            ...
            // Reset all state to default so we do not have unexpected variable assignments
            ...
            focusedState.Apply = () =>
            {
                //  When focused, use the focused animatiuon and enable animation playback
                background.CurrentChainName = focusedAnimation.Name;
                background.Animate = true;
            };

            // Create the highlight+focused state for mouse hover while focused
            ...
            // by cloning the enable state since they appear the same
            ...
            // Add event handle for mouse hover focused
        ...
        {
            if(e.Key == Gum.Forms.Input.Keys.Left)
            {
                // Left arrow navigates to previous contrrol
                HandleTab(TabDirection.Up, loop: true);
            }
            if(e.Key == Gum.Forms.Input.Keys.Right)
            {
                // Right arow navigates to next control
