...
namespace SlimeGame.GameObjects
{
    ...
    public class Slime
    {
        ...
        // The amount of time that has elapsed since the last movement updated.
        ...
        //Event that is raised if it is detected that the head segm,ent of the slime
        // has collided with a body segment
        public event EventHandler BodyCollision;

        // Creates a new Slime using the specified sprite. 
        ...
        public void Initialize(Vector2 startingPosition, float stride)
        {
            ...
            // Create the inital head of the slime chain
            ...
            //Set the initial direction as the same direction the head is moving
        ...
        private void HandleInput()
        {
            ...
            // If a new direction was input, consdering adding it to the buffer
            if (potentialNextDirection != Vector2.Zero && _inputBuffer.Count < MAX_BUFFER_SIZE)
            {
                // If the buffer is empty, validate agasinst the current direction;
                // otherwise, validate against the last buffered direction
                Vector2 valiadateAgainst = _inputBuffer.Count > 0 ?
                                            _inputBuffer.Last() :
                ...
                float dot = Vector2.Dot(potentialNextDirection, valiadateAgainst);
                if (dot >= 0)
                {
        ...
        private void Move()
        {
            ...
            // Insert the new adjusted value for the head at the front of 
            // the segments and remove the tail segment. This effectively moves
            // the entire chain forward without needing to loop through every 
            // segment and uipdates its "at" and "to" positions
            _segments.Insert(0, head);
            _segments.RemoveAt(_segments.Count - 1);

            // Iterate through all the segments except the head and check
            // if they are at the same position as the head. IFthey are, 
            // if the head is colliding with a body segment and a body collision
            // has occurred
        ...
        public void Grow()
        {
            ...
            // Creates a new tail segment that is positioned a grid cell in 
            // the reverse direction from the tail moving to the tail
        ...
        public void Update(GameTime gameTime)
        {
            ...
            // If the movement timer has accumlated enough time to be great than
        ...
        public void Draw()
        {
            ...
            {
                // Calculate the visual position of the segment at the moment
                // by lerping between its "at" and "to" position by the movement
                // offset lerp amount
                Vector2 pos = Vector2.Lerp(segment.At, segment.To, _movementProgress);

                //Draw the slime sprite at the caclulated visual position of this segment
